...
public class TelemetryClientFactory {
    ...
    private EventBatchSender buildEventBatchSender(DaemonConfig config) throws MalformedURLException {
        var eventsConfig = EventBatchSenderFactory.fromHttpImplementation(httpPosterCreator)
    ...
    private MetricBatchSender buildMetricBatchSender(DaemonConfig config) throws MalformedURLException {
        var metricConfig = MetricBatchSenderFactory.fromHttpImplementation(httpPosterCreator)
                .configureWith(config.getApiKey())
                .secondaryUserAgent(makeUserAgent(config))
                .auditLoggingEnabled(true);
