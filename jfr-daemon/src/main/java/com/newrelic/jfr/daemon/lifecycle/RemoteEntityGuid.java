...
public class RemoteEntityGuid {
     ...
     * the entity guid. If the remote MBean is not found, this method will return null.
     * When the linking metadata does not yet contain an entity guid, then this
     * code will enter a busy-wait loop, checking every second forever until the entity guid is
     * contained in the linking metadata.
     *
     * @return The entity guid of the remote service or null if the remote service does not expose the expected MBean.
     */
    public String queryFromJmx() {
        Callable<Result> callable = this::tryToFetchGuid;
        Predicate<Result> completionCheck = result -> result.type == Result.Type.NO_AGENT || result.type == Result.Type.OK;
        var busyWait = new BusyWait<>("Getting entity guid", callable, completionCheck);
